...
ADR-001, ADR-002 y ADR-004 a ADR-008 tienen estado `Aceptado — revisión 4`; ADR-003, `Aceptado — revisión 5`; ADR-009, `Aceptado — revisión 3`; ADR-011 y ADR-012, `Aceptado — revisión 1`; ADR-010 está reservado para policy packs. El plan del Incremento 1 está aprobado en revisión 7. I1-H01 está `DONE / ACCEPTED` y no debe volver a implementarse; su evidencia se conserva en la [aceptación de I1-H01](reviews/aceptacion-i1-h01.md). La [evaluación de C0_CATALOG](reviews/c0-catalog-assessment.md) considera satisfecha la base general de reutilización de información pública y mantiene el gate `PENDING` por incertidumbres materiales de adquisición y semántica. I1-H02 permanece `BLOCKED`, no `READY`; `S0_PUBLIC_ENDPOINTS` permanece `PENDING` y ninguna nueva historia está autorizada todavía. H01–H05 se aceptan localmente sin AWS según ADR-012. `CL0_CLOUD_STAGING` permanece `PENDING` y solo se cierra después de desplegar y verificar H05 con la fixture sintética exclusiva de staging.
