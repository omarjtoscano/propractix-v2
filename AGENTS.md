...
- `C0_CATALOG` está `CLOSED` por aprobación expresa del Product Owner el 2026-09-21 sobre el commit `e98e11655da7a8f37b6c26d180d6b7ca2d3e8e52`. I1-H02 permanece `BLOCKED` únicamente por `S0_PUBLIC_ENDPOINTS`; ninguna nueva historia está autorizada todavía.
...
- La baseline usa `eu-north-1`, una `t4g.small` ARM64, ECR privado, Parameter Store Standard, S3, EBS `gp3` de 16 GiB y OpenTofu. No presupongas que créditos equivalen a recursos sin coste.
- Bajo la baseline aprobada sin NAT, IPv6 ni endpoints VPC, la EC2 necesita una IPv4 pública dinámica para egress hacia SSM, ECR y repositorios de paquetes. No reserves EIP ni abras ingress; el acceso inicial usa SSM/port forwarding.
...
- `staging` utiliza exclusivamente la fixture `docs/policies/staging-synthetic-privacy-fixture-v1.yaml`: nunca datos personales reales, nunca exportación y nunca promoción a producción.
- AWS es infraestructura: ningún dominio o caso de uso importa SDKs o conceptos del proveedor.

## Desarrollo local y ramas

- Sigue ADR-012: H01 a H05 se desarrollan y aceptan sin AWS con PostgreSQL 16 y Mailpit; los secretos locales son sintéticos y se montan como archivos consumidos mediante Spring config tree.
- LocalStack no forma parte del perfil local por defecto. Solo puede arrancarse en el perfil opcional `aws-contract` para probar contratos de adapters AWS; no es requisito de aceptación de H01 a H05.
- `main` será la rama estable y predeterminada. Hasta que exista, `increment/01-company-identity-catalog` conserva la línea aprobada; crear/cambiar la rama predeterminada es una acción administrativa explícita, no parte de una historia ni de este cambio documental.
- Después de crear `main`, no hagas push directo: usa pull request, checks obligatorios y ramas `increment/*` o `feature/*`; nunca force push ni borrado de `main`.
